...
4. **사내 주소를 커밋하지 마십시오.** endpoint 는 환경변수로 줍니다.
   문서와 예시에는 `<your-llm-host>` 같은 자리표시자를 쓰십시오.
   실제로 `tests/test_live_llm.py` 의 실행 예시에 사설 IP 가 박혀 커밋된 적이
   있습니다. 한 번 올라가면 히스토리에 남아 되돌리기 어렵습니다.
   `python tests/run_all.py` 의 "사설 IP 노출 검사" 가 이것을 잡습니다.
   정당한 경우는 줄 끝에 `gxpllm-allow-private-ip` 로 예외 처리하십시오.
